...
import { MINISTRIES, getOptions, DEFAULT_OPTIONS } from './Dashboard_p1';
...
export default function ConstitutionModal({ country, onSave, onClose }) {
    ...
    const [activeMin,       setActiveMin]       = useState(
        new Set(current.ministries || globalGov.ministries)
    );
    const [crisisMin,       setCrisisMin]       = useState(
        current.crisis_ministry !== undefined ? current.crisis_ministry : globalGov.crisis_ministry
    );
    ...
    const toggleMinistry = (id) => {
        setActiveMin(prev => {
            ...
            if (next.has(id)) {
                if (next.size <= 2) return prev; // minimum 2 ministères
                next.delete(id);
            } else {
                next.add(id);
            }
    ...
    // ── Déterminer si diff avec global ──
    const isDiff = !useGlobal || (
