        ...
        <div ref={scrollRef} style={S.body}>
          ...
          {agents && (<>
                ...
                <section key={m.id} style={{...S.sec,opacity:activeMins.includes(m.id)?1:0.38}}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:'0.22rem'}}>
                    {Object.entries(agents.ministers).map(([key,min])=>{
                      const isIn=m.ministers.includes(key);
                      return (
                        <button key={key} style={{...BTN_S,fontSize:'0.42rem',padding:'0.16rem 0.42rem',
                          ...(isIn?{borderColor:min.color+'88',color:min.color,background:min.color+'14'}:{})}}
                          onClick={()=>setAgents(a=>({...a,ministries:a.ministries.map((x,i)=>i!==mi?x:{
                            ...x,ministers:isIn?x.ministers.filter(k=>k!==key):[...x.ministers,key]
                          })}))}>
                          {min.emoji} {min.name}
                        </button>
                      );
            ...
            {tab==='ministers' && (<>
              ...
              {Object.entries(agents.ministers).map(([key,min])=>(
                <section key={key} style={S.sec}>
                  <div style={{display:'flex',alignItems:'center',gap:'0.38rem',marginBottom:'0.18rem'}}>
                    <input style={{...INPUT,width:'1.95rem',textAlign:'center',fontSize:'1rem',padding:'0.15rem',flexShrink:0}} value={min.emoji} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],emoji:e.target.value}}}))}/>
                    <input style={{...INPUT,flex:1,fontSize:'0.52rem'}} value={min.name} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],name:e.target.value}}}))}/>
                    <input type="color" value={min.color} style={{width:'1.9rem',height:'1.8rem',border:'none',background:'none',cursor:'pointer',flexShrink:0}} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],color:e.target.value}}}))}/>
                  </div>
                  <div style={S.label}>ESSENCE</div>
                  <textarea style={{...INPUT,minHeight:'34px',resize:'vertical',lineHeight:1.5,fontFamily:FONT,marginBottom:'0.26rem'}} value={min.essence||''} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],essence:e.target.value}}}))}/>
                  <div style={S.label}>STYLE DE COMMUNICATION</div>
                  <textarea style={{...INPUT,minHeight:'26px',resize:'vertical',lineHeight:1.5,fontFamily:FONT,marginBottom:'0.26rem'}} value={min.comm||''} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],comm:e.target.value}}}))}/>
                  <div style={{...S.label,display:'flex',alignItems:'center',gap:'0.4rem',flexWrap:'wrap'}}>
                    ANGLE D'ANNOTATION
                    <span style={{fontFamily:FONT,fontSize:'0.37rem',color:'rgba(140,160,200,0.35)',fontWeight:'normal',letterSpacing:'0'}}>— question posée lors des annotations inter-ministérielles</span>
                  </div>
                  <textarea style={{...INPUT,minHeight:'30px',resize:'vertical',lineHeight:1.5,fontFamily:FONT}} value={min.annotation||''} onChange={e=>setAgents(a=>({...a,ministers:{...a.ministers,[key]:{...a.ministers[key],annotation:e.target.value}}}))}/>                </section>
              ))}
