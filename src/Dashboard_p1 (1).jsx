...
export const PAYS_LOCAUX = STATS.pays_locaux;
...
export function buildCountryPrompt(type, nomDemande = '') {
  return `Tu es un générateur de données géopolitiques pour un jeu de simulation politique.
Crée un pays ${type === 'reel' ? 'réel existant' : 'fictif crédible et original'}.
${type === 'reel' && nomDemande ? `Nom demandé : ${nomDemande}` : ''}
...
Réponds UNIQUEMENT en JSON valide, sans markdown, sans commentaires, sans texte avant ou après.
{
  ...
  "emoji": "un seul emoji représentatif du pays",
  "couleur": "#RRGGBB — couleur sombre et saturée, visible sur fond noir",
  "regime": "une valeur parmi : democratie_liberale | republique_federale | monarchie_constitutionnelle | technocratie_ia | junte_militaire | oligarchie | theocratie",
  "terrain": "une valeur parmi : coastal | inland | island | archipelago | highland",
  "description": "une phrase immersive de 15-25 mots, ton journalistique",
  "leader": {
    "nom": "prénom et nom du dirigeant actuel",
    "titre": "titre officiel (ex: Premier Ministre, Roi, Général...)",
    "trait": "une phrase décrivant sa philosophie de gouvernance"
  },
  "population": "entier entre 500000 et 80000000",
  "tauxNatalite": "float entre 6.0 et 35.0",
  "tauxMortalite": "float entre 4.0 et 18.0",
  "satisfaction": "entier entre 20 et 85",
  "ressources": {
...
/** Construit le prompt IA pour un événement narratif de seuil critique */
...
async function callModel(model, prompt, keys, systemPrompt = '') {
  ...
  if (model === 'gemini' && keys.gemini) {
    try {
      ...
      console.warn('[ARIA] Gemini error:', e.message);
      return { error: true, msg: getRandomFallback() };
    }
  }
...
export async function callAI(prompt, type = 'standard', context = {}) {
  ...
  switch (type) {
    ...
    default:
      return callModel(roles.solo_model || 'claude', prompt, keys);
  }
}
...
export function getOptions() {
  try {
    ...
    return {
      ...
      api_keys: { ...DEFAULT_OPTIONS.api_keys, ...saved.api_keys },
...
export function useARIA({ setSelectedCountry, isCrisis }) {
  // ── State ──────────────────────────────────────────────────────────────────
  const [phase,      setPhase]      = useState('init');       // 'init' | 'game'
  const [worldData,  setWorldData]  = useState(null);         // topographie immuable
  const [countries,  setCountries]  = useState([]);           // pays actifs
  const [alliances,  setAlliances]  = useState([]);           // [{ a, b, type }]
  const [events,     setEvents]     = useState([]);           // log narratif
  const [aiRunning,  setAiRunning]  = useState(false);        // loading IA
  const [notification, setNotif]   = useState(null);          // toast courant
  ...
  const startLocal = useCallback((W = 1400, H = 800) => {
    const seed  = Math.floor(Math.random() * 999999);
    const world = initWorld(seed, W, H);
    const built = PAYS_LOCAUX.map(t => buildCountryFromLocal(t, world));
    setCountries(built);
    setAlliances([]);
    setEvents([]);
    setPhase('game');
    pushNotif('Mode local activé — Valoria, Eldoria, Thalassia sont prêts.', 'ok');
  }, [initWorld, pushNotif]);
  ...
  const startWithAI = useCallback(async (countryDefs, W = 1400, H = 800) => {
    ...
    for (const def of countryDefs) {
      try {
        ...
        if (aiData) {
          built.push(buildCountryFromAI(aiData, world, built));
        } else {
          // Fallback local si l'IA échoue sur ce pays
          const fallback = PAYS_LOCAUX[built.length % PAYS_LOCAUX.length];
          built.push(buildCountryFromLocal(fallback, world));
  ...
  const advanceCycle = useCallback(async () => {
    const keys     = getApiKeys();
    const updated  = countries.map(c => doCycle(c, alliances));
    const triggers = [];
  ...
  // ── Sécession ─────────────────────────────────────────────────────────────
  const doSecession = useCallback(async (parentId, nomNouveau, relationType) => {
    ...
    // Nouveau pays adjacent au parent
    const jitter = parent.size * 1.4;
    const rand   = seededRand(seed);
    const newCx  = parent.cx + randRange(rand, -jitter, jitter);
    const newCy  = parent.cy + randRange(rand, jitter * 0.5, jitter * 1.2);
    const newSize = parent.size * 0.55;

    // Parent perd 25% de population et ressources
    const updatedParent = {
      ...parent,
      population:   Math.round(parent.population * 0.75),
      satisfaction: Math.max(parent.satisfaction - 8, 10),
      size:         parent.size * 0.88,
      // Path réduit vers le centroïde (scale visuelle)
      ...
      ressources:   Object.fromEntries(
        Object.entries(parent.ressources).map(([k, v]) =>
          [k, v && rand() > 0.35]   // perd ~35% des ressources
        )
      ),
    };
    ...
    // Nouveau pays fils
    const childCountry = {
      id:           nomNouveau.toLowerCase().replace(/[^a-z0-9]/g, '-'),
      nom:          nomNouveau,
      emoji:        '🆕',
      couleur:      `hsl(${Math.floor(rand() * 360)}, 55%, 38%)`,
      regime:       parent.regime,
      regimeName:   parent.regimeName,
      regimeEmoji:  parent.regimeEmoji,
      ...
      popularite:   55,
      ressources:   calcRessources(parent.terrain, seed),
      ...
      influenceRadius: calcInfluenceRadius(Math.round(parent.population * 0.25), parent.coastal, calcRessources(parent.terrain, seed)),
  ...
  const resetWorld = useCallback(() => {
    ...
    pushNotif('Monde réinitialisé.', 'info', 2000);
  }, [setSelectedCountry, pushNotif]);
