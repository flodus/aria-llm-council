...
export function useARIA({ setSelectedCountry, isCrisis, onReset }) {
    ...
    const startLocal = useCallback((customDefs = null, W = 1400, H = 800) => {
        ...
        // customDefs = defs passées par InitScreen (mode perso hors ligne)
        // Sans defs = défaut : Valoria seul
        const templates = customDefs
        ? customDefs.map(d => {
            // 1. Chercher dans PAYS_LOCAUX par id
            const local = PAYS_LOCAUX.find(p => p.id === d.realData?.id);
            if (local) return local;
            // 2. Si pays réel (realData avec population) → l'utiliser directement comme template
            if (d.realData?.population) return d.realData;
            // 3. Fallback
    ...
    const resetWorld = useCallback(() => {
        ...
        onReset?.();
    }, [setSelectedCountry, pushNotif, onReset]);
