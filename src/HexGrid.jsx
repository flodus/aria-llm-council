...
function countryNeon(country) {
    if (isValidHex(country?.couleur)) return country.couleur;
...
function HexGridInner({
    ...
    const handleClick = useCallback((id) => {
        ...
        return (
            ...
            {/* ClipPaths pour les masses terrestres (hex côtiers) */}
            {world.masses && world.masses.map(mass => (
                hulls.get(mass.id)
                ? <clipPath key={`cp-${mass.id}`} id={`clip-${mass.id}`}>
                <polygon points={hulls.get(mass.id)} />
                </clipPath>
                : null
            ))}
                    ...
                    .map(pl => {
                        ...
                        return (
                            ...
                            {isHov && (
                                ...
                                stroke={hexRgba(neon, 0.45)} strokeWidth="0.8" />
                                ...
                                return (
                                    ...
                                    {pl.hexKeys.map(k => {
                                        ...
                                        return (
                                            <polygon key={k}
                                            points={hexPointsStr(h.x, h.y)}
                                            fill={hexRgba(neon, 0.22)}
                                            stroke={hexRgba(neon, 0.65)}
                                            strokeWidth="0.7"
                                            />
                                        );
                                    })}
                                    ...
                                    stroke={hexRgba(neon, 0.40)} strokeWidth="1" />
                                    {/* Filet coloré en haut */}
                                    <rect x={pl.cx-70} y={pl.cy-45} width={140} height={2}
                                    rx="1" fill={hexRgba(neon, 0.55)} />
