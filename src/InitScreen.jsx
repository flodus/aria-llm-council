...
function APIKeyInline({ onClose }) {
    ...
    const save = () => {
        try {
            ...
            // Persiste le statut pour que Settings affiche "Connecté" directement
            const s = { claude: status.claude === 'ok' ? 'ok' : null, gemini: status.gemini === 'ok' ? 'ok' : null };
    ...
    return (
        ...
        <button style={BTN_PRIMARY} onClick={save}>SAUVEGARDER</button>
        </div>
        </div>
        </div>
    );
...
function CountryConfig({ c, idx, mode, onChange, onRemove, canRemove }) {
    ...
    return (
        ...
        {c.type === 'reel' && mode === 'ai' && (() => {
            ...
            r.id === c.nom.toLowerCase().replace(/[^a-z]/g,'')
            );
            return (
                ...
                {knownMatch
                    ...
                    : c.nom && <div style={{ fontSize:'0.43rem', color:'rgba(100,120,160,0.50)', fontStyle:'italic', lineHeight:1.5 }}>
...
export default function InitScreen({ worldName, setWorldName, onLaunchLocal, onLaunchAI, hasApiKeys, onRefreshKeys }) {
            ...
            if (t >= 30) {
                ...
                if (usePreset === 'defaut_local') {
                    ...
                    const defs = (customDefs || countries).map(c => ({
                        type: c.realData ? 'reel' : 'imaginaire',
                        nom:  c.nom || 'Nation',
                        regime:   c.regime,
                        terrain:  c.terrain,
    ...
    if (step === 'config') {
        ...
        if (preset === 'custom') return (
            ...
            <button style={BTN_PRIMARY} onClick={() => launch('custom')}>GÉNÉRER LE MONDE →</button>
            </div>
            </div>
        );
