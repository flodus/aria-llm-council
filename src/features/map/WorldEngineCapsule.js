// WorldEngineCapsule.js — copie capsule pour proto ma-planete
// NE PAS MODIFIER WorldEngine.js — ce fichier est la source ARIA

// ═══════════════════════════════════════════════════════════════════════════
//  WorldEngineCapsule.js  —  Moteur de génération du monde ARIA
//
//  Pipeline :
...
function buildLandmassHulls(grid, masses) {
  const hulls = new Map(); // massId → SVG path string (outline réel = hard-edged)

  for (const mass of masses) {
    // Utilise le vrai contour des bords extérieurs des hex → effet low-poly angulaire
    const keySet = new Set(mass.members);
    const edges  = [];
    ...
    for (const key of mass.members) {
      ...
      nbrs.forEach(([nc, nr], dir) => {
        if (keySet.has(`${nc},${nr}`)) return; // voisin dans la masse → pas de bord
    ...
    // Chaîner les segments en polylines fermées
    const EPS = 0.5;
    ...
    // Convertir en path SVG — lignes droites (L) = hard-edged low-poly
    const pathStr = chains.map(ch =>
      ch.map((e, i) => i === 0
...
export const CAPSULE_SOURCE  = 'WorldEngine.js @ aria v7.5';
