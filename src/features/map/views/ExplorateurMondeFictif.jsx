...
export default function ExplorateurMondeFictif({ seed = 42, onMondeReel }) {
  ...
  const allerVers = useCallback((nouvelleVue, options = {}) => {
    clearTimeout(timerRef.current)
    if (nouvelleVue === 'globe') {
      // Retour globe : enlever overlay, lancer morph retour
      setVue('globe')
      setIsPlanar(false)
      setPays(null)
    } else {
      // Aller vers mercator/warroom : déclencher morph, puis overlay après délai
      if (options.pays !== undefined) setPays(options.pays)
      setIsPlanar(true)
      timerRef.current = setTimeout(() => setVue(nouvelleVue), DUREE_MORPH)
    }
  }, [])

  const nouveauMonde = useCallback(() => {
    clearTimeout(timerRef.current)
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', backgroundColor: '#020208' }}>

      {/* Canvas toujours monté — morph globe ↔ mercator */}
      <div style={{ position: 'absolute', inset: 0 }}
        onDoubleClick={vue === 'globe' && !isPlanar ? () => allerVers('mercator') : undefined}>
        <Canvas camera={{ position: [0, 0, RAYON_GLOBE * 4.2], fov: 42 }}>
          <color attach="background" args={['#020208']} />
          <Etoiles count={50000} rayon={150} />
          <GlobeFictif seed={localSeed} isPlanar={isPlanar} />
          {!isPlanar && <OrbitControls enableZoom enablePan={false} rotateSpeed={0.8} zoomSpeed={1.0} />}
        </Canvas>
      </div>

      {/* Overlay mercator / warroom */}
      {vue !== 'globe' && (
        <MondeFictif
          seed={localSeed}
          ...
          onPaysDoubleClick={idx => allerVers('warroom', { pays: idx })}
        />
      )}

      {/* Toolbar globe uniquement */}
      {vue === 'globe' && !isPlanar && (
